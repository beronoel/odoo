...
class TaxOutputter:
    def __init__(self, runbot_instance_name, output_prefix):
        self.url = 'http://localhost:8069' #'http://'+runbot_instance_name+'.runbot7.odoo.com'
        try:
            urllib2.urlopen(self.url)
        except urllib2.HTTPError, err:
            if err.code == 502:
                self.url = 'http://'+runbot_instance_name+'.runbot6.odoo.com'
        ...
        #self.dbname = runbot_instance_name + '-all'
    ...
    def setUpChart(self, chart_template):
        ...
        name = 'Test E' + chart_template['name']
        ...
        # Create fiscal year
        fiscalyear_id = self.execute('account.fiscalyear', 'create', {'name': '2015', 'code': '2015', 'date_start': '2015-01-01', 'date_stop': '2015-12-31', 'company_id': self.company_id})
        try:
            self.execute('account.fiscalyear', 'create_period', [fiscalyear_id])
        except:
            pass
    ...
    def check_taxes(self, op_type):
        ...
        if op_type == 'sale':
            ...
            ref_journal_id = self.execute('account.journal', 'search', [('type', '=', 'sale_refund'), ('company_id', '=', self.company_id)])[0]
            partner_id = self.execute('res.partner', 'search', [('name', '=', 'Agrolait')])[0]
            account_id = self.execute('account.account', 'search', [('type', '=', 'receivable')])[0]
            ...
            ref_journal_id = self.execute('account.journal', 'search', [('type', '=', 'purchase_refund'), ('company_id', '=', self.company_id)])[0]
            partner_id = self.execute('res.partner', 'search', [('name', '=', 'China Export')])[0]
            account_id = self.execute('account.account', 'search', [('type', '=', 'payable')])[0]
            inv_type = 'in_invoice'
        ...
        master_inv_line_id = self.execute('account.invoice.line', 'create', {'name': 'toto', 'invoice_id': master_invoice_id, 'price_unit': 1000})
        ...
        master_ref_line_id = self.execute('account.invoice.line', 'create', {'name': 'toto', 'invoice_id': master_refund_id, 'price_unit': 1000})
        #prepare the csv file to store the accounting entries to compare
        with open(csv_file, 'wb') as f:
            writer = csv.writer(f, delimiter=',')
            writer.writerow(['tax code', 'TYPE (invoice/refund)', 'account_id', 'debit', 'credit', 'invoice amount untaxed', 'invoice amount tax', 'invoice total', 'vat declaration', 'tax_amount'])
            #search all the taxes
            tax_ids = self.execute('account.tax', 'search', [('type_tax_use', 'in', tax_types), ('parent_id', '=', False)])
            #tmp = 0
            for tax_id in tax_ids:
                ...
                self.execute('account.invoice.line', 'create', {'name': 'toto', 'invoice_id': tmp_invoice_id, 'price_unit': 1000, 'invoice_line_tax_id': [(6, 0, [tax_id])]})
                ...
                self.execute('account.invoice.line', 'create', {'name': 'toto', 'invoice_id': tmp_refund_id, 'price_unit': 1000, 'invoice_line_tax_id': [(6, 0, [tax_id])]})
                ...
                for obj_id in [tmp_invoice_id, tmp_refund_id]:
                    writer.writerow([])
                    invoice_data = self.execute('account.invoice', 'read', [obj_id], ['id', 'amount_untaxed', 'amount_tax', 'amount_total', 'type'])[0]
                    aml_ids = self.execute('account.move.line', 'search', [('invoice', '=', obj_id)])
                    l = self.execute('account.move.line', 'read', aml_ids, ['id', 'name', 'account_id', 'debit', 'credit', 'tax_amount', 'tax_code_id'])
                    for aml_data in sorted(l, key=lambda row: (row['debit'],row['credit'],row['tax_code_id'])):
                        #aml_data.values() + invoice_data.values()
                        sign = '+'
                        if aml_data['tax_code_id'] and aml_data.get('tax_amount',0) < 0:
                            sign ='-'
                        vals = [tax_code, invoice_data['type'], aml_data['account_id'][1].split(' ')[0], aml_data['debit'], aml_data['credit'], invoice_data['amount_untaxed'], invoice_data['amount_tax'], invoice_data['amount_total'], aml_data['tax_code_id'] and ('[\'' +sign+'BETAX'+aml_data['tax_code_id'][1].split(' - ')[0]+'\']') or '[]', abs(aml_data['tax_amount'])]
            ...
            #    invoice_data = self.execute('account.invoice', 'read', [obj_id], ['amount_untaxed', 'amount_tax', 'amount_total', 'type'])[0]
            #    aml_ids = self.execute('account.move.line', 'search', [('invoice', '=', obj_id)])
            #    l = self.execute('account.move.line', 'read', aml_ids, ['id', 'name', 'account_id', 'debit', 'credit', 'tax_amount', 'tax_code_id'])
            #    for aml_data in sorted(l, key=lambda row: (row['debit'],row['credit'],row['tax_code_id'])):
            #        vals = ['master', invoice_data['type'], aml_data['name'], aml_data['account_id'], aml_data['debit'], aml_data['credit'], aml_data['tax_amount'], aml_data['tax_code_id'], invoice_data['amount_untaxed'], invoice_data['amount_tax'], invoice_data['amount_total']]
            #        writer.writerow([unicode(s).encode('utf-8') for s in vals])
            #return csv_file
...
if __name__ == '__main__':
    ...
    v8_outputter = TaxOutputter(sys.argv[1], 'v8')
    # new_design_outputter = TaxOutputter(sys.argv[2], 'new_design')

    chart_templates_ids = v8_outputter.execute('account.chart.template', 'search', chart_templates_domain, 0, 9999, "id asc")
    chart_templates = v8_outputter.execute('account.chart.template', 'read', chart_templates_ids, ['id', 'name', 'complete_tax_set', 'code_digits'])

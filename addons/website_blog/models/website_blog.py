    ...
    def _get_teaser(self):
        for blog_post in self:
            if blog_post.teaser_manual:
                ...
                teaser_split = []
                for node in html.fragment_fromstring(blog_post.content, create_parent='div').xpath('//p'):
                    teaser_split.extend(filter(None, node.text.replace('\n', ' ').split(' ')))
                    if len(teaser_split) >= 50:
                        break
                blog_post.teaser = ' '.join(teaser_split[:50]) + '...'

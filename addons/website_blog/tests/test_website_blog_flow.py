...
class TestWebsiteBlogFlow(TestWebsiteBlogCommon):
    ...
    def test_website_blog_followers(self):
        ...
        self.assertIn(
            self.user_blogmanager.partner_id, test_blog.message_follower_ids.mapped('partner_id'),
        ...
        self.assertNotIn(
            self.user_employee.partner_id, test_blog_post.message_follower_ids.mapped('partner_id'),
            'website_blog: subscribing to a blog should not subscribe to its posts')
        self.assertNotIn(
            self.user_public.partner_id, test_blog_post.message_follower_ids.mapped('partner_id'),
            'website_blog: subscribing to a blog should not subscribe to its posts')

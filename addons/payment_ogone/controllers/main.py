...
class OgoneController(http.Controller):
    ...
    def ogone_s2s_create_json(self, **kwargs):
        new_id = request.env['payment.acquirer'].browse(int(kwargs.get('acquirer_id'))).sudo().s2s_process(kwargs)
        return new_id

    @http.route(['/payment/ogone/s2s/create'], type='http', auth='public', methods=["POST"], csrf=False)
    def ogone_s2s_create(self, **post):
        request.env['payment.acquirer'].browse(int(post.get('acquirer_id'))).sudo().s2s_process(post)

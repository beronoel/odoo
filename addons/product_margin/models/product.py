...
class ProductTemplate(models.Model):
    ...
    total_margin = fields.Float(compute='_compute_product_margin')

    @api.multi
    def _compute_product_margin(self):
        margin_data = self.env['report.product.margin'].read_group([('product_tmpl_id', 'in', self.ids)], ['total_margin','product_tmpl_id'],['product_tmpl_id'])
...
class Product(models.Model):
    ...
    @api.multi
    def _compute_product_margin(self):
        margin_data = self.env['report.product.margin'].read_group([('product_id', 'in', self.ids)], ['total_margin','product_id'],['product_id'])

...
class Invite(models.TransientModel):
    ...
    def add_followers(self):
        ...
        for wizard in self:
            ...
            new_partners = wizard.partner_ids - document.message_follower_ids.mapped('partner_id')
            new_channels = wizard.channel_ids - document.message_follower_ids.mapped('channel_id')
            ...
            if wizard.send_mail and wizard.message and not wizard.message == '<br>':  # when deleting the message, cleditor keeps a <br>
                ...
                signature_company = self.env['res.partner']._get_signature_footer(user_id=self._uid, res_model=wizard.res_model, res_id=wizard.res_id)

...
class TestMessagePost(TestMail):
    ...
    def test_post_no_subscribe_author(self):
        # TDE note: use partner_1 to avoid group specific access rights and behavior
        original = self.partner_1.message_follower_ids
        self.partner_1.sudo(self.user_employee).with_context({'mail_create_nosubscribe': True}).message_post(
            body='Test Body', message_type='comment', subtype='mt_comment')
        self.assertEqual(self.partner_1.message_follower_ids.mapped('partner_id'), original.mapped('partner_id'))
        self.assertEqual(self.partner_1.message_follower_ids.mapped('channel_id'), original.mapped('channel_id'))
    ...
    def test_post_subscribe_author(self):
        # TDE note: use partner_1 to avoid group specific access rights and behavior
        original = self.partner_1.message_follower_ids
        self.partner_1.sudo(self.user_employee).message_post(
            body='Test Body', message_type='comment', subtype='mt_comment')
        self.assertEqual(self.partner_1.message_follower_ids.mapped('partner_id'), original.mapped('partner_id') | self.user_employee.partner_id)
        self.assertEqual(self.partner_1.message_follower_ids.mapped('channel_id'), original.mapped('channel_id'))
    ...
    def test_post_subscribe_recipients(self):
        # TDE note: use partner_1 to avoid group specific access rights and behavior
        original = self.partner_1.message_follower_ids
        self.partner_1.sudo(self.user_employee).with_context({'mail_create_nosubscribe': True, 'mail_post_autofollow': True}).message_post(
            body='Test Body', message_type='comment', subtype='mt_comment', partner_ids=[(4, self.partner_1.id), (4, self.partner_2.id)])
        self.assertEqual(self.partner_1.message_follower_ids.mapped('partner_id'), original.mapped('partner_id') | self.partner_1 | self.partner_2)
        self.assertEqual(self.partner_1.message_follower_ids.mapped('channel_id'), original.mapped('channel_id'))
    ...
    def test_post_subscribe_recipients_partial(self):
        # TDE note: use partner_1 to avoid group specific access rights and behavior
        original = self.partner_1.message_follower_ids
        self.partner_1.sudo(self.user_employee).with_context({'mail_create_nosubscribe': True, 'mail_post_autofollow': True, 'mail_post_autofollow_partner_ids': [self.partner_2.id]}).message_post(
            body='Test Body', message_type='comment', subtype='mt_comment', partner_ids=[(4, self.partner_1.id), (4, self.partner_2.id)])
        self.assertEqual(self.partner_1.message_follower_ids.mapped('partner_id'), original.mapped('partner_id') | self.partner_2)
        self.assertEqual(self.partner_1.message_follower_ids.mapped('channel_id'), original.mapped('channel_id'))

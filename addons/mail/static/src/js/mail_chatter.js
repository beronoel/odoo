...
var ChatterMailComposeMessage = mail_thread.MailComposeMessage.extend({
    ...
    message_post: function(body, attachment_ids, values){
        ...
        // update subtype
        if(this.is_log) {
            var subtype_id = parseInt(this.$('.o_mail_chatter_compose_message_subtype_select').first().val());
 ...
 **/
var DocumentMailThread = form_common.AbstractField.extend(mail_thread.MailThreadMixin, {
    ...
    /**
     * Constructor of the 'mail_thread' widget
     * The specific options of the node can be :
     *  - 'display_log_button': display of the 'log a note' button
     *  - 'display_document_link': display document link on message
     *  - 'internal_subtypes': list of internal subtype (for employee only)
     *  - 'emoji_list': list of emoji
     */
    ...
    on_message_show_more: function(event){
        var self = this;
        this.message_fetch(this.get_message_domain_history()).then(function(messages){
     ...
     * @param message_id : the identifier of the new posted message
     */
    ...
    render_value: function(){
        ...
        this.message_format(this.get_value().slice(0, mail_thread.LIMIT_MESSAGE)).then(function(messages){
            self.set('messages', self._message_preprocess(messages));
            self.record_name = _.last(messages) ? _.last(messages).record_name : '';
        });
    },
    // composer toggle
...
core.form_widget_registry.add('mail_thread', DocumentMailThread);
...
return {
    DocumentMailThread: DocumentMailThread,
};

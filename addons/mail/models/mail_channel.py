...
class Channel(models.Model):
    ...
    # description
    channel_name = fields.Char('Name', required=True, translate=True)  # TDE MIGRATION: was "name" for mail.channel
    channel_type = fields.Selection([
        # ('personal', 'Personal'),
        ('chat', 'Chat Discussion'),
        ('channel', 'Channel')], 'Channel Type', default='channel')
    ...
    alias_name = fields.Char('Alias Name', related='alias_id.alias_name', store=True)
    alias_contact = fields.Selection('Alias Name', related='alias_id.alias_contact', store=True)
    alias_domain = fields.Char('Alias Domain', related='alias_id.alias_domain', store=True)
    ...
    def create(self, vals):
        alias_values = dict((key, vals[key]) for key in vals.keys() if key.startswith('alias_'))
        alias = self.env['mail.alias'].with_context(alias_model_name=self._name, alias_parent_model_name=self._name).create(alias_values)
    ...
    def _subscribe_users(self):
        for channel in self:
            channel.write({'channel_partner_ids': [(4, pid) for pid in channel.mapped('group_ids').mapped('users').mapped('partner_id').ids]})
        return True
    ...
    def message_post(self, body='', message_type='notification', subtype='mail.mt_comment', content_subtype='html', **kwargs):
    ...
    #   - when joining channel (only to the new added partners)
    #   - when creating a channel (only to the creator)
    ...
    def _notify(self, message):
        ...
        """
        message.ensure_one()
        message_values = message.message_read()['threads'][0][0] #TODO JEM : remove useless field to broadcast such as author_avatar

        print message.message_format()
        print "========"
        ...
        for channel in self:
            ...
            if partner_channels.ids:
                ...
                if len(partner_channel) >= 1:
                    ...
                    info['state'] = partner_channel.fold_state
    ...
    def channel_fetch_message(self, last_id=False, limit=20):
        ...
        threads = self.env['mail.message'].message_read_wrapper(domain=domain, limit=limit)['threads']
        if not threads:
            return []
        return threads[0]
        ...
        if partners_to:
            ...
            if result:
                ...
                channel = self.create({
                    ...
                    'public': 'private',
                    'channel_type': 'chat',
                    'channel_name': ', '.join(self.env['res.partner'].sudo().browse(partners_to).mapped('name')),
                })
    ...
    def channel_seen(self):
        self.ensure_one()
        self.env['mail.channel.partner'].search([('channel_id', 'in', self.ids), ('partner_id', '=', self.env.user.partner_id.id)]).write({'seen_datetime': time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)})
    ...
    def channel_invite(self, partner_ids):
        ...
        for partner in partners:
            user_id = partner.user_ids[0] # TODO JEM : is it save ? may throw exception ?
    ...
    def get_init_notifications(self):
        """ Get unread messages and old messages received less than AWAY_TIMER
            ...
            Note : the user need to be logged
        """
        # NOTE JEM : just find the init notifications of the minified conversation !!
        ...
        message_values = self.env['mail.message'].message_read(domain=domain, thread_level=0)
        if len(message_values['threads']):
            message_values = message_values['threads'][0]
        else:
            message_values = []
    ...
    def channel_fetch_slot(self):
        ...
        pinned_channel_ids = pinned_channels.ids
        ...
        direct_message_channels = self.search([('channel_type', '=', 'chat'), ('id', 'in', pinned_channel_ids)])
    ...
    @api.model
    def search_to_join(self, name=None, domain=None):
        """ Return the channel info of the channel the current partner can join
            ...
            :returns dict : channel header
        """
        if not domain:
            domain = []
        domain += [('channel_type', '=', 'channel'), ('channel_partner_ids', 'not in', [self.env.user.partner_id.id])]
        if name:
            domain.append(('channel_name', 'ilike', name))
        return self.search(domain).read(['channel_name', 'public', 'uuid', 'channel_type'])

    @api.multi
    def channel_join_and_get_info(self):
        self.ensure_one()
        self.action_follow()
        # broadcast and return it to the current user
    ...
    @api.model
    def channel_create(self, channel_name, privacy='public'):
        ...
        """
        # create the channel
        new_channel = self.create({'channel_name': channel_name, 'public': privacy})

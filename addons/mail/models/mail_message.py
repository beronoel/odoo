...
class Message(models.Model):
    ...
    _message_read_fields = ['id', 'parent_id', 'model', 'res_id', 'body', 'subject', 'date', 'email_from',
        'message_type', 'attachment_ids', 'tracking_value_ids', 'author_id', 'partner_ids', 'record_name']
    ...
    def _notify(self, force_send=False, user_signature=True):
        ...
        partners._notify(self)  # TDE: clean those parameters
        channels._notify(self)

        # compute partners to notify
        # TDE CHECK: cgheck email_from and things like that ?
        # print '\tnotifying channels', channels, 'and partners', partners
        ...
        self.env['res.partner'].sudo().search([
            ...
            ('notify_email', '!=', 'none')]
        ).send_notification_email(self, force_send=force_send, user_signature=user_signature)  # TDE: clean those parameters

        # print partners, channels
        vals = {'channel_ids': [(6, 0, channels.ids)], 'needaction_partner_ids': [(6, 0, partners.ids)]}
        self.write(vals)
        return True

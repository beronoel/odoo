...
class Partner(models.Model):
    ...
    def _get_signature_footer(self, user_id, res_model=None, res_id=None, user_signature=True):
    ...
    def _notify_email(self, message, force_send=False, user_signature=True):
        ...
        signature_company = self._get_signature_footer(user_id, res_model=message_sudo.model, res_id=message_sudo.res_id, user_signature=(user_signature and message_sudo.model != 'mail.channel'))
    ...
    @api.multi
    def send_notification_email(self, message, force_send=False, user_signature=True):
        if self.env.context.get('mail_notify_noemail'):
            return True
        self._notify_email(message, force_send=force_send, user_signature=user_signature)
        return True

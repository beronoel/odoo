...
class QWeb(orm.AbstractModel):
    ...
    def render_attribute(self, element, name, value, qwebcontext):
        ...
        if not context.get('rendering_bundle'):
            ...
            elif request and getattr(request, 'website', False) and request.website.cdn_activated and (name == self.URL_ATTRS.get(element.tag) or name == self.CDN_TRIGGERS.get(element.tag)):
    ...
    def render_text(self, text, element, qwebcontext):
        compress = request and not request.debug and getattr(request, 'website', False) and request.website.compress_html

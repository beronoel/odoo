...
class PaymentTransaction(models.Model):
    ...
    def form_feedback(self, data, acquirer_name):
        ...
        if tx and tx.state == 'done' and tx.acquirer_id.auto_confirm == 'at_pay_confirm' and tx.sale_order_id and tx.sale_order_id.state in ['draft', 'sent']:
            tx.sale_order_id.sudo().with_context(send_email=True).action_button_confirm()
        elif tx and tx.state != 'cancel' and tx.sale_order_id and tx.sale_order_id.state == 'draft':

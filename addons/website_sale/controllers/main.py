...
class WebsiteSale(http.Controller):
    ...
    def get_attribute_value_ids(self, product):
        ...
        if request.website.pricelist_id.id != request.context['pricelist']:
            ...
            for p in product.product_variant_ids:
                price = website_currency_id.compute(p.lst_price, currency_id)
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        products = ProductTemplate.with_context(local_context).search(domain, limit=PPG, offset=pager['offset'], order='website_published desc, website_sequence')
        attributes = request.env['product.attribute'].search([('attribute_line_ids.product_tmpl_id', 'in', products.ids)])
    ...
    def product(self, product, category='', search='', **kwargs):
        ...
        ratings = request.env['rating.rating'].search([('message_id', 'in', product.website_message_ids.ids)])
    ...
    def pricelist(self, promo, **post):
        pl = request.env['product.pricelist'].sudo().search([('code', '=', promo)], limit=1)
        if pl:
            if not request.website.is_pricelist_available(pl[0]):
    ...
    def checkout_values(self, data=None):
        ...
        if not data:
            if request.uid != request.website.user_id.id:
                ...
                shipping_ids = Partner.sudo().search([("parent_id", "=", partner.id), ('type', "=", 'delivery')]).ids
            else:
                order = request.website.sale_get_order(force_create=1)
                if order.partner_id:
    ...
    def checkout_form_save(self, checkout):
        ...
        if checkout.get('shipping_id') == -1:
            ...
            checkout['shipping_id'] = Partner.create(shipping_info).id
    ...
    def confirm_order(self, **post):
        ...
        extra_step = request.env.ref('website_sale.extra_info_option', raise_if_not_found=True)
    ...
    def extra_info(self, **post):
        ...
        if 'post_values' in post:
            ...
            for field_name, field_value in post.items():
                if field_name in SaleOrder._fields and field_name.startswith('x_'):
    ...
    def payment(self, **post):
        ...
        values['errors'] = order._get_errors(order)
        values.update(order._get_website_data(order))
        ...
        if not values['errors']:
            ...
            for acquirer in values['acquirers']:
                acquirer.button = acquirer.with_context(submit_class='btn btn-primary', submit_txt=_('Pay Now')).render(
                    order.name,
                    order.amount_total,
    ...
    def payment_validate(self, transaction_id=None, sale_order_id=None, **post):
        ...
        if sale_order_id is None:
            ...
            order = SaleOrder.sudo().browse(sale_order_id)
            assert order.id == request.session.get('sale_last_order_id')
    ...
    def get_unit_price(self, product_ids, add_qty, use_order_pricelist=False, **kw):
        ...
        prices = request.env['product.pricelist'].price_rule_get_multi([(product, add_qty, partner) for product in products])

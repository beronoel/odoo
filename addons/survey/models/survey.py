...
class Survey(models.Model):
    ...
    def filter_input_ids(self, filters, finished=False):
        ...
        if finished:
            ...
            return user_inputs.filtered(lambda input: input.state == 'done').ids
...
class SurveyPage(models.Model):
    ...
    title = fields.Char(string='Page Title', required=True, translate=True)
    ...
    description = fields.Html(translate=True, oldname="note", help="An introductory text to your page")
...
class SurveyQuestion(models.Model):
    ...
    # Validation
    validation_required = fields.Boolean(string='Validate entry', oldname='is_validation_require')
    ...
    validation_error_msg = fields.Char(string='Error message', oldname='validation_valid_err_msg',
...
class SurveyLabel(models.Model):
    ...
    question_id_2 = fields.Many2one('survey.question', string='Question', ondelete='cascade')
...
class SurveyUserInput(models.Model):
    ...
    date_create = fields.Datetime(string='Creation Date', default=fields.Datetime.now, required=True, readonly=True, copy=False)
    deadline = fields.Datetime(help="Date by which the person can open the survey and submit answers",
    ...
    token = fields.Char(string='Identification token', default=lambda self: uuid.uuid4().__str__(),
    ...
    result_url = fields.Char(related='survey_id.result_url', string="Public link to the survey results")
    print_url = fields.Char(related='survey_id.print_url', string="Public link to the empty survey")

    quizz_score = fields.Float(compute="_compute_quizz_score", default='0.0', string="Score for the quiz")

    @api.depends('user_input_line_ids.quizz_mark')
    def _compute_quizz_score(self):
        for input in self:
            input.quizz_score = sum(input.user_input_line_ids.mapped('quizz_mark'))
...
class SurveyUserInputLine(models.Model):
    ...
    quizz_mark = fields.Float(string="Score given for this choice")

...
class MrpProduction(models.Model):
    ...
    def onchange_product_id(self):
        if not self.product_id:
            self.write({'product_uom_id': False, 'bom_id': False, 'routing_id': False, 'product_tmpl_id': False})
        else:
            bom_point = self.env['mrp.bom']._bom_find(product=self.product_id, properties=[])
            routing_id = False
            if bom_point:
                routing_id = bom_point.routing_id
            self.write({'product_uom_id': self.product_id.uom_id and self.product_id.uom_id.id or False, 'bom_id': bom_point.id,
                        'routing_id': routing_id and routing_id.id or False, 'product_tmpl_id': self.product_id.product_tmpl_id.id})

...
class TestOrderProcess(TestOrderDemo):
    ...
    # I check production lines after compute.
        self.assertEqual(len(self.mrp_production_test1.product_line_ids), 5, "Production lines are not generated proper.")
   ...
   # I check details of Produce Move of Production Order to trace Final Product.
    ...
    # I consume raw materials and put one material in scrap location due to waste it.
        ...
        for move in self.mrp_production_test1.move_line_ids:
    ...
    #I check procurements have been generated for every consume line
        ...
        move_line_ids = [x.id for x in self.mrp_production_test1.move_line_ids]
        ...
        for proc in self.env['procurement.order'].browse(procurements):
            for order_line in self.mrp_production_test1.product_line_ids:
                ...
                if proc.product_id.id == order_line.product_id.id:
                    ...
                    self.assertEqual(proc.location_id.id, order.location_src_id.id, "Input location and procurement location do not correspond")
                    self.assertEqual(proc.product_uom_id.id, order_line.product_uom_id.id, "UOM does not correspond in procurement.")
    ...
    # I produce product.
        ...
        lines = self.mrp_product_produce1.on_change_qty(qty, [])
        self.mrp_product_produce1.write(lines['value'])
    ...
    # I check Total Costs at End of Production as a manager.
        ...
        for workcenter_line in self.mrp_production_test1.workcenter_line_ids:

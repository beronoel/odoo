...
def call_kw(model, name, args, kwargs):
    ...
    adapter = CALL_KW_ADAPTERS.get(getattr(method, '_api', None))
    if not adapter:
        raise TypeError("Method %s.%s may not be called remotely", model._name, method.__name__)
    return adapter(method, model, args, kwargs)

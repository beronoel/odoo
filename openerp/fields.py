...
class Field(object):
    ...
    def __get__(self, record, owner):
        ...
        try:
            return record._cache[self.name]
        except KeyError:
            pass
            ...
            try:
                ...
                for record in records:
                    try:
                        ...
                        record._cache.set_failed(exc, self.name)
    ...
    def get_value(self, record):
        ...
        if self.column and not (self.depends and env.in_onchange):
                ...
                if recs:
                    ...
                    if recs.env != env:
                        ...
                        for source, target in zip(recs, recs.with_env(env)):
                            try:
                                target._cache.update(target._convert_to_cache({
            ...
            record._cache[self.name] = self.null(env)
...
class _RelationalMulti(_Relational):
    ...
    def _update(self, records, value):
        ...
        for record in records:
            if self.name in record._cache:
                record._cache[self.name] = record[self.name] | value
            else:
    ...
    def _update_getter(self, record, value):
        def getter():
            ...
            result = record._cache[self.name] = record[self.name] | value
            return result
        return getter

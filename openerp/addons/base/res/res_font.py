...
class ResFont(models.Model):
    ...
    _sql_constraints = [
        ('name_font_uniq', 'unique(family, name)',
            _('You can not register two fonts with the same name'))
    ]
    ...
    def font_scan(self, lazy=False):
        ...
        if lazy:
            ...
            found_fonts = self.search([('path', '!=', '/dev/null')])
            if not found_fonts:
    ...
    def _scan_disk(self):
        ...
        inexistant_fonts = self.search([('name', 'not in', existing_font_names),
                                        ('path', '!=', '/dev/null')])
        if inexistant_fonts:
            inexistant_fonts.unlink()

        RegistryManager.signal_caches_change(self.env.cr.dbname)
        self._sync()
        return True

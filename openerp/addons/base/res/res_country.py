...
def location_name_search(self, name='', args=None, operator='ilike', limit=100):
    ...
    record_ids = []
    if len(name) == 2:
        record_ids = self.search([('code', 'ilike', name)] + args, limit=limit).ids

    search_domain = [('name', operator, name)]
    if record_ids:
        search_domain.append(('id', 'not in', record_ids))
    record_ids.extend(self.search(search_domain + args, limit=limit).ids)

    locations = []
    for record in record_ids:
        locations.append(self.browse(record).name_get()[0])
    return sorted(locations, key=lambda (id, name): record_ids.index(id))
...
(for example, use '%(street)s' to display the field 'street') plus
    ...
    country_group_ids = fields.Many2many('res.country.group', 'res_country_res_country_group_rel',
                                         'res_country_id', 'res_country_group_id',
...
class CountryGroup(models.Model):
    ...
    country_ids = fields.Many2many('res.country', 'res_country_res_country_group_rel',
                                   'res_country_group_id', 'res_country_id',
                                   string='Countries')

...
class ResPartnerCategory(models.Model):
    ...
    def _check_recursion(self):
        if not super(ResPartnerCategory, self)._check_recursion():
            raise UserError(_('Error ! You can not create recursive tags.'))
...
class ResPartner(models.Model, format_address):
    ...
    def _display_name_compute(self):
        for partner in self:
            ...
            res = dict(partner.with_context(context).name_get())
        return res
    ...
    def _commercial_partner_id(self):
        partner_ids = self._commercial_partner_compute()
        for res in self:
            res.commercial_partner_id = partner_ids[res.id]

    @api.depends('parent_id', 'is_company', 'name')
    def _display_name(self):
        for res in self:
            res.display_name = res._display_name_compute()[res.id]
    ...
    display_name = fields.Char(compute='_display_name', string='Name', store=True, index=True)
    ...
    lang = fields.Selection(_lang_get, 'Language', default=api.model(lambda self: self.env.lang), help="If the selected language is loaded in the system, all documents related to this contact will be printed in this language. If not, it will be English.")
    tz = fields.Selection(_tz_get, 'Timezone', default=api.model(lambda self: self.env.context.get('tz', False)), help="The partner's timezone, used to output proper date and time values inside printed reports. It is important to set a value for this field. You should use the same timezone that is otherwise used to pick and render date and time values: your computer's timezone.")
    ...
    category_id = fields.Many2many('res.partner.category', id1='partner_id', id2='category_id', string='Tags', default=_default_category)
    ...
    use_parent_address = fields.Boolean(string='Use Company Address', help="Select this if you want to set company's address information  for this contact")
    ...
    commercial_partner_id = fields.Many2one('res.partner', compute=_commercial_partner_id, string='Commercial Entity', store=True)
    ...
    def _get_default_image(self, partner_type, is_company, parent_id):
        ...
        if not image and partner_type == 'invoice':
            ...
            img_path = odoo.modules.get_module_resource('base', 'static/src/img', 'company_image.png')
        elif not image:
            img_path = odoo.modules.get_module_resource('base', 'static/src/img', 'avatar.png')
            colorize = True
    ...
    def onchange_parent_id(self):
        ...
        if not self.parent_id or not self.ids:
            return {'value': {}}
        if self.parent_id:
            result = {}
            ...
            if self.type == 'contact':
                ...
                if any(self.parent_id[key] for key in address_fields):
                    result['value'] = dict((key, value_or_id(self.parent_id[key])) for key in address_fields)
        return result
    ...
    def _commercial_sync_from_company(self):
        ...
        if not commercial_partner:
            ...
            commercial_partner_id = self._commercial_partner_compute()[self.id]
    ...
    def _commercial_sync_to_children(self):
        ...
        return self.browse([c.id for c in sync_children]).write(sync_vals)
    ...
    def _fields_sync(self, update_values):
        ...
        if self.child_ids:
            ...
            if any(field in update_values for field in address_fields):
                domain_children = [('parent_id', '=', self.id), ('type', '=', 'contact')]
                update_ids = self.search(domain_children)
    ...
    def open_parent(self):
        ...
        partner = self.browse(self.ids[0])
        address_form_id = self.env['ir.model.data'].xmlid_to_res_id('base.view_partner_address_form')
        return {'type': 'ir.actions.act_window',
                'res_model': 'res.partner',
                'view_mode': 'form',
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        active = dict(self.env.context or {})
    ...
    def find_or_create(self, email):
        ...
        if emails:
            email = emails[0]
        record = self.search([('email', '=ilike', email)], limit=1)
        if not record:
    ...
    def address_get(self, adr_pref=None):
        ...
        default = result.get('contact', self.ids and self.ids[0] or False)
    ...
    def view_header_get(self, view_id, view_type):
        res = super(ResPartner, self).view_header_get(view_id=view_id, view_type=view_type)
        if res:
            return res
        if not self.env.context.get('category_id', False):
            return False
        return _('Partners: ')+self.env['res.partner.category'].browse(self.env.context['category_id']).name
